...
import (
	...
	"github.com/rao-manish-24/Needflicks/Server/NeedflicksServer/models"
	"github.com/rao-manish-24/Needflicks/Server/NeedflicksServer/utils"
...
func GetUsersFavouriteGenres(userId string, client *mongo.Client, c *gin.Context) ([]string, error) {
	...
	return extractGenreNames(favGenresArray), nil
...
func extractGenreNames(favGenresArray bson.A) []string {
		...
		case bson.D:
			for _, elem := range g {
				if elem.Key == "genre_name" {
					if name, ok := elem.Value.(string); ok && name != "" {
						genreNames = append(genreNames, name)
					}
				}
			}
		case bson.M:
			if name, ok := g["genre_name"].(string); ok && name != "" {
				genreNames = append(genreNames, name)
			}
		case map[string]any:
			if name, ok := g["genre_name"].(string); ok && name != "" {
